...
function initStickyCategoryNav() {
    ...
    if (mobileNavQuery.addEventListener) mobileNavQuery.addEventListener("change", measure);
...
function initSearchHistory() {
    ...
    dropdown.addEventListener("click", function (e) {
        ...
        if (e.target.closest(".delete-search, .history-remove")) {
            var removeButton = e.target.closest(".delete-search, .history-remove");
            var removeTerm = removeButton.dataset.term;
        ...
        var itemButton = e.target.closest(".recent-search-item, .search-history-item, .history-item, .suggestion-item");
        if (itemButton) {
...
function removeOrderedCartItems(orderedItems) {
    var orderedIds = (orderedItems || []).map(function (item) {
...
function getCsrfToken() {
    ...
    return (meta && meta.content) || (document.body && document.body.dataset.csrf) || "";
}
...
async function createPhonePePayment(items, mode, form, paymentMethod) {
    ...
    try {
        ...
        if (result.success && result.redirect_url) {
            sessionStorage.setItem("zafiroPendingOrderMode", mode);
            sessionStorage.setItem("zafiroPendingOrderItems", JSON.stringify(items));
            markCheckoutScrollReset();
...
async function createFinalOrder(items, mode, paymentMethod, form) {
    ...
    try {
        var response = await fetch("save-order.php", {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-CSRF-Token": getCsrfToken() },
        ...
        var result = await response.json();
        if (!result.success) {
            showToast(result.message || "Order could not be saved. Cart was not changed.", "error");
            return;
        }
    } catch (error) {
        showToast("Order could not be saved. Cart was not changed.", "error");
        return;
    }
